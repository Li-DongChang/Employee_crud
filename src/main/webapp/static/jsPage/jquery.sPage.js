    ...
    $.extend(Plugin.prototype, {
        ...
        creatHtml: function (i) {
            if (i == this.settings.page) {
                this.pageList.push('<span class="active" data-page='+i+' onclick=getAllEmp('+i+');>' + i + '</span>');
            } else {
//                this.pageList.push('<span data-page=' + i + '>' + i + '</span>');
                this.pageList.push('<span data-page='+i+' onclick=getAllEmp('+i+');>' + i + '</span>');
            }												
//            							'onclick = "getAllEmp('++')"'
//            						'<span data-page="+i+" onclick="alert(1);">'
        },
        viewHtml: function () {
            ...
            if (settings.showPN) {
                settings.page == 1 ? this.pageList.push('<span class="span-disabled" data-page="prev">' + settings.prevPage + '</span>') : this.pageList.push('<span data-page="prev" onclick=getAllEmp('+(settings.page-1)+')>' + settings.prevPage + '</span>');
            }
            if (pageTatol <= 6) {
                ...
                if (settings.page < 5) {
                    ...
                    this.pageList.push('<span data-page="none">...</span><span data-page=' + pageTatol + ' onclick=getAllEmp('+pageTatol+')>' + pageTatol + '</span>');
                } else if (settings.page > pageTatol - 4) {
                    this.pageList.push('<span data-page="1" onclick="getAllEmp(1);">1</span><span data-page="none">...</span>');
...
//                    													('<span data-page='+i+' onclick=getAllEmp('+i+');>' + i + '</span>');
//                  this.pageList.push('<span data-page="none">...</span><span data-page=' + pageTatol + '>' + pageTatol + '</span>');  
                  this.pageList.push('<span data-page="none">...</span><span data-page=' + pageTatol + ' onclick=getAllEmp('+pageTatol+')>' + pageTatol + '</span>');
                }
            }
            if (settings.showPN) {
                settings.page == pageTatol ? this.pageList.push('<span class="span-disabled" data-page="next">' + settings.nextPage + '</span>') : this.pageList.push('<span data-page="next" onclick=getAllEmp('+(settings.page+1)+')>' + settings.nextPage + '</span>');
            }
            pageArr.push(this.pageList.join(''));
            pageArr.push('</div>');
            if (settings.showSkip) {
                pageArr.push('<div class="spage-skip">跳至&nbsp;<input type="text" id="toPage"/>&nbsp;页&nbsp;&nbsp;<span data-page="go" onclick=getAllEmp($("#toPage").val())>确定</span></div>');
            }
